...
- After a successful preview comment, [`.github/scripts/preview-notify-linked-issues.js`](../.github/scripts/preview-notify-linked-issues.js) upserts the same body on issues linked via `Refs #NNN` (or legacy closing keywords; not `Part of #NNN`) and may move Project 4 Status to `In QA` (optional `PROJECT_BOARD_TOKEN`)
