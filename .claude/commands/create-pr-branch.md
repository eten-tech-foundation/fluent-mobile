...
Run with `/create-pr-branch` in Cursor chat. Canonical file: this one.
`/start-issue` **reads and executes** these naming steps (always from `main`)
instead of re-specifying slug/type logic.
...
Execute these steps in order. Do not skip validation or user confirmation before
creating the branch (except when invoked from `/start-issue`, which already is
the explicit ask — still print the branch name).
...
- If the proposed branch **already exists**, stop and tell the user (when called
  from `/start-issue`, resume that branch instead — see start-issue step 4c).
...
- If not on `main`, note the current branch and confirm they want to branch from here.
  `/start-issue` always bases on `origin/main`.
...
Prefer creating from `origin/main` without updating local `main` when a
checkout of `main` is blocked:

```bash
git fetch origin main
git checkout -b "{author}/{type}/{issue-number}-{slug}" origin/main
```

Report success and remind them to use `/create-pr` (or `/start-issue` will chain
it) when ready.

## Integration

- Pairs with `/create-pr`, `/open-pr` (alias), and `/generate-pr-description`.
- `/start-issue` reuses this command for naming + branch creation.
